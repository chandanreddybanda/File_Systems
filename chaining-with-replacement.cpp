...
void insert(int a[][4],int ele)
{
	...
	while(1)
	{
			...
			{
				a[loc][1]=ele;
				fout.seekp(loc*7,ios::beg);
				fout<<ele;
				a[loc][0]=1;
			}
			else if(a[loc][0]==1 && a[loc][2]==-1)
			{
			  if(a[loc][1]%10==ele%10)
			 { 
			  ...
			  {
			  	int t1=a[loc][1],t2=a[loc][3],temp,count=0;
					a[loc][1]=ele;
					fout.seekp(loc*7,ios::beg);
					fout<<ele;
					a[loc][3]=-1;
					while(count!=10 && a[loc][0]!=0)
					{
						loc=(loc+1)%10;
						count++;
					}
			...
			else if(a[loc][0]==1 && a[loc][2]!=-1)
			{
				...
				{
					int t1=a[loc][1],t3=a[loc][3],t2=a[loc][2],count=0;
					a[loc][1]=ele;
					fout.seekp(loc*7,ios::beg);
					fout<<ele;
					a[loc][3]=-1;a[loc][2]=-1;
...
{
	ofstream fout;
	fout.open("linear2.txt",ios::trunc);
